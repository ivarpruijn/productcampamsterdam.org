<?php
    ...
    if($_GET['liveServerDeployComplete']) {
        add_action('init', create_function('','flush_rewrite_rules();echo 1;exit;'));
    }
    ...
    function liveServerDeploy_automate($settings) {
        ...
        $mysql_exec = LSD_MYSQL_DUMP_USE_EXEC;// || $capabilities['mysql_exec']; // if we can dump using exec then thats all we need
        $mysql_dump = true;
    ...
    function liveServerDeploy_check_permissions($path, $ignore_list = array()) {
        $permissions_ok = true;
        $dirh = opendir($path);
        while ($file = readdir($dirh)) :
            if($file == '.' || $file == '..' || $file == 'lsd_dump.zip' || $file == 'lsd_dump.sql') continue;
            if(array_search($file, $ignore_list) !== false || array_search($path.'/'.$file, $ignore_list) !== false || array_search($path.'/'.$file."/", $ignore_list) !== false) {
                continue;
            }
            
            if(!is_readable($path.'/'.$file)) {
                liveServerDeploy_update_status("[Permissions Error] ".$path.'/'.$file." is not readable", 'error');
                $permissions_ok = false;
            } else if(!@liveServerDeploy_check_permissions($path.'/'.$file, $ignore_list))
                $permissions_ok = false;
        endwhile;
