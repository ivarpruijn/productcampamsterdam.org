...
<script>
    function liveServerDeploy_tab(tab) {
        ...
        jQuery('.wrap #tab-'+tab).show();
        jQuery('.nav-tab').removeClass('nav-tab-active');
        jQuery('.nav-tab#'+tab).addClass('nav-tab-active');
        return false;
    }
    
    var tab = window.location.hash.toString();
    if(tab) tab = tab.substr(1);
    else tab = "main";
    liveServerDeploy_tab(tab);
</script>
<div class="wrap">
...
<form method=post>
    ...
    <table class="form-table">
	    ...
	    <tr>
		    ...
		    <td>
		        <input name="ignore_list" id="ignore_list" type="text" value="<?php echo esc_attr($ignore_list); ?>" class="regular-text code"><em>Make a list of directories or file names to ignore. Put between double quotes or comma seperate (If the file includes a comma then put between double quotes). e.g <code>.gitignore, .git, .svn, "cache"</code></em><br>
		    </td>
	    </tr>
	    </tbody>
    </table>
...
<div class="tab" id="tab-export" style="display:none">
<h3>Manual Export</h3>
